...
class PlayControllerTest(@Autowired val restTemplate: TestRestTemplate) {
    ...
    fun `Assert api returns string with expected winner player two and winning action`() {
        val response =
            restTemplate.getForEntity<String>("/api/play?playerOneAction={playerOneAction}&playerTwoAction={playerTwoAction}", mapOf("playerOneAction" to "ROCK", "playerTwoAction" to "PAPER"))
        Assertions.assertThat(response.statusCode).isEqualTo(HttpStatus.OK);
        Assertions.assertThat(response.body).isEqualTo("Player Two won with action ${Action.PAPER} against ${Action.ROCK}");
    }
